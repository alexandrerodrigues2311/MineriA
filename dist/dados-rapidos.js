...
window.fastJSON=async function(url){
 ...
 if(supported.has(path)&&typeof DecompressionStream!=='undefined'){
  try{const r=await fetch(path+'.gz'+(query?'?'+query:''));if(!r.ok)throw Error();const bytes=new Uint8Array(await r.arrayBuffer());const stream=new Blob([bytes]).stream();return normalizeMinerals(JSON.parse(await new Response(bytes[0]===31&&bytes[1]===139?stream.pipeThrough(new DecompressionStream('gzip')):stream).text()),url)}catch(e){/* Retry the same snapshot without compression. */}
 }
